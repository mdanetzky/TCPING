...
void usage(int argc, char* argv[]) {
    ...
    cout << " --fail : automatically exit on a failed ping"<<endl;                  //[Modification 14 Apr 2019 by Matthias Danetzky, matthias.danetzky@gmail.com]
...
int main(int argc, char* argv[]) {
	...
	for (int x = 0; x < argc; x++) {
		...
		// exit on first failed ping
		if (!strcmp(argv[x], "--fail")) {
			auto_exit_on_failure = 1;
			offset = x;
		}
	...
	if (!reading_from_file) {
		retval = DoWinsock_Single(pcHost, nPort, times_to_ping, ping_interval, include_timestamp, beep_mode, ping_timeout, relookup_interval, auto_exit_on_success, auto_exit_on_failure, force_send_byte, include_url, use_http, docptr, http_cmd, include_jitter, jitter_sample_size, logfile, use_logfile, ipv, proxy_server, proxy_port, using_credentials, proxy_credentials, only_changes, no_statistics, giveup_count, out, use_source_address, src_address, blocking, always_print_domain, use_color);
	}
	else {
		...
		times_to_ping = 1;
		retval = DoWinsock_Multi(pcHost, nPort, times_to_ping, ping_interval, include_timestamp, beep_mode, ping_timeout, relookup_interval, auto_exit_on_success, auto_exit_on_failure, force_send_byte, include_url, use_http, docptr, http_cmd, include_jitter, jitter_sample_size, logfile, use_logfile, ipv, proxy_server, proxy_port, using_credentials, proxy_credentials, only_changes, no_statistics, giveup_count, file_times_to_loop, urlfile, out, use_source_address, src_address, blocking, always_print_domain, use_color);
	}
